...
Bivariate normality  seems to be violated. For comparison, below graph \@ref(fig:perfectbinorm).depicts a bivarite normal distribution with r=0.7. Nevertheless, for illustrative purposes we use these data to test the null hypothesis $H_0: \rho=0$ against the non-directional alternative hypothesis $H_1: \rho \neq0$.The scatter plot is provided in \@ref(fig:scatterincgenatt).
...
The income variable was sligthly skewed due to a small number of relatively large income values. In fact, when the World Bank team analyzed the data using a regression, they top-coded and transformed the income variable (for details @Hirshleifer16).
...
When the data is in the rank format, or there is a need for protection against outliers^[Here protection refers to being less sensetive to outliers compared to Pearson coefficient. However Spearman's rho and Kendall's tau might be more sensetive to outliers compared to robust procedures, see @wilcox2012.] when working with continuous data the Spearman correlation coefficient is used. If the number of ties in the ranks is not large, procedures provided for the Pearson correlation coefficient can be utilized. Setting the method argument to "spearman", the _cor.test_ function first transforms the data into ranks and performs the procedures introduced for the Pearson coefficient. 
...
The association between a continuous variable and a dichotomously reflected latent continious variable can be examined with a biserial correlation. In psychometrics , for example, biserial correlation is used for calculating the correlation between a total test score (continuous) and  a dichotomous item score (assumed to underlie a latent variable).
...
__Causation__ A correlation coefficient does not imply causation.  For any correlation there are at least four possible interpretations involving cuasation: (a) X causes Y, (b) Y causes X,  (c) both X and Y  share one or more common causes, and (d) X and Y have different causes, but these causes are correlated.
...
Now suppose the correlation is between GRE scores and GPA.  The correlation between GRE scores and GPA is typically somewhere between .10 and .30, so a .60 correaltion would be a very large correlation coefficient.
...
Its possible to correct for attenuation using $$r_{T_x T_y}=\frac{r_{xy}}{\sqrt(r_{xx}r_{yy})}$$ where $r_{xx}$ and $r_{yy}$ is the reliability coefficients.
